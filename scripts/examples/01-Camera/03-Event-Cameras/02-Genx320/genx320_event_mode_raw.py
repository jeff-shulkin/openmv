...
# Shape: bytearray of length (EVT_res * 6 * sizeof(np.uint16)) where EVT_res is the event resolution
# EVT_res: must be a power of two between 1024 and 65536.
EVT_res = 2048 # 2048 event long buffer
uint16_size = 2 # Each np.uint16 is 2 bytes large
event_buffer = bytearray(EVT_res * 6 * uint16_size)
...
while True:
    ...
    event_count = csi0.ioctl(csi.IOCTL_GENX320_READ_EVENTS_RAW, event_buffer)
